# frontend-bundler Specification

## Purpose

定义浏览器内基于 esbuild-wasm 的完整工程打包能力，用于将功能开发节点生成的工程文件打包为可在预览 iframe 中执行的 ES module bundle。覆盖内存文件系统模块解析、Vue SFC 编译、第三方依赖外置、增量重建与防抖，以及各类失败场景下的降级与静默策略。
