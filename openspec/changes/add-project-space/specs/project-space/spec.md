...
- **WHEN** 携带 token 请求 `GET /api/v1/projects`(不带 team_id)
...
- **WHEN** 携带 token 请求 `GET /api/v1/teams`
- **THEN** 返回当前用户已加入的全部团队

#### Scenario: 搜索团队

- **WHEN** 请求 `GET /api/v1/teams/search?keyword=<名称关键字>`
- **THEN** 返回名称匹配的团队列表,排除当前用户已加入的团队

#### Scenario: 加入团队

- **WHEN** 携带 token 请求 `POST /api/v1/teams/:id/join`
