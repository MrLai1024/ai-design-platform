...
export function listTeams(): Promise<Team[]> {
  return get<Team[]>('/v1/teams');
}

/**
 * 按名称搜索团队(排除当前用户已加入的团队)
 * GET /api/v1/teams/search?keyword= → Team[]
 */
export function searchTeams(keyword: string): Promise<Team[]> {
  return get<Team[]>('/v1/teams/search', { keyword });
}
...
export function joinTeam(teamId: string): Promise<void> {
  return post<void>(`/v1/teams/${teamId}/join`);
}
