...
export async function compileProject(projectRoot, opts = {}) {
  ...
  // Quick-check entry tolerance: the entry (src/main.ts / App.vue) is
  // generated by a LATER task — the bootstrap entry task sits at the END of
  // the dependency chain. Mid-generation quick checks must not fail the
  // current task over it: like unresolved imports, a missing entry is
  // dependency-not-ready, not a defect. The candidates ride along so the
  // planning-gap check can replan them if no task produces them. (Empty
  // project still errors — nothing to build at all; full checks still hard-
  // fail on a missing entry.)
  if (!full) {
    const files = collectFiles(projectRoot)
    const hasEntry = Boolean(entry) || files['src/main.ts'] || files['App.vue']
    if (Object.keys(files).length > 0 && !hasEntry) {
      return { ok: false, errors: [], partial: true, partial_paths: ['src/main.ts', 'App.vue'] }
    }
  }
